...
function getPedestrianGenderQuery(nestedWhere, startYear, endYear) {
    ...
    SELECT code, juriCount, ROUND(JuriCount * 100.0 / JuriTotal, 2) AS MuniPercent, ROUND(StateCount * 100.0 / StateTotal, 2) AS StatePercent FROM (
    SELECT State.code, JuriCount, StateCount AS StateCount FROM
    (SELECT (CASE sex
                WHEN 'F' THEN 'Female'
                WHEN 'M' THEN 'Male'
                ELSE 'Unknown'
...
function getPedestrianAgeQuery() {
    ...
    SELECT code, juriCount, ROUND(JuriCount * 100.0 / JuriTotal, 2) AS MuniPercent, ROUND(StateCount * 100.0 / StateTotal, 2) AS StatePercent FROM (
    SELECT State.code, JuriCount, StateCount AS StateCount FROM
    (SELECT AgeBucket AS Code, count(*) AS MuniCount FROM (select 
        CASE
        ...
        AS AgeBucket
        FROM ard_pedestrians where ${nestedWhere} and acc_year >= ${startYear} and acc_year <= ${endYear}
        GROUP BY AgeBucket ORDER BY AgeBucket) AS Juri
    RIGHT JOIN
    (SELECT code, SUM(statecount) AS StateCount FROM ${stateTable}
    WHERE year >= ${startYear} AND year <= ${endYear} GROUP BY code ORDER BY code) AS State
    ON Juri.code = State.code
...
function getPedestrianPreCrashQuery() {
    ...
    SELECT code, juriCount, ROUND(JuriCount * 100.0 / JuriTotal, 2) AS MuniPercent, ROUND(StateCount * 100.0 / StateTotal, 2) AS StatePercent FROM (
        SELECT State.type AS Code, 
        CASE 
            WHEN MuniCount > 0 THEN MuniCount
            ELSE 0
            END AS MuniCount,
        CASE
            WHEN StateCount > 0 THEN StateCount 
            ELSE 0
            END AS StateCount 
        FROM 
            (
                ...
                ELSE pre_crash_type
                END), count(*) AS MuniCount 
                FROM ard_pedestrians
                WHERE ${nestedWhere}  AND (acc_year >= ${startYear} and acc_year <= ${endYear})
                GROUP BY pre_crash_type ORDER BY pre_crash_type) AS preCrashCount
                LEFT JOIN
