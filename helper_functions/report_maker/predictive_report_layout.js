...
// *---------------*
function makePredictiveReport(queryArgs, reportData, reportTitle, reportSaveName, subFolder) {
    const output_folder = path.join(base_path, '..\\..\\output', subFolder);

    return new Promise((resolve, reject) => {
        try {
            const savePath = path.join(output_folder, reportSaveName);
            const fileInfo = {
                savePath: savePath,
                fileName: reportSaveName
            };
...
function getTableTitleHeader(doc, tableTitle, yPos) {
    ...
    if (tableTitle) {
        doc.setFont('seguisb', 'normal').setFontSize(14).setTextColor(104, 104, 104).text(tableTitle, leftMargin, yPos);
        currYPos += 2;
        doc.setDrawColor(104, 104, 104)
            .setLineWidth(0.5)
            ...
            didParseCell: function (data) {
                ...
                if (crashAttr === 'default') {
                    if (
                        ...
                        (data.column.dataKey === 'fatal' || data.column.dataKey === 'incapacitated')
                    ) {
...
function createReportFilterLabels(queryStrings) {
    ...
    if (queryStrings.sri) location = queryStrings.sriName + ' (' + queryStrings.sri + ')';
        ...
        const juriCode = `${
            queryStrings.muniCode ? queryStrings.countyCode + queryStrings.muniCode : queryStrings.countyCode
        }`;
...
function getTableColumns(crashAttr) {
    ...
    else if (crashAttr === 'surf_cond_code')
        cols = [
    ...
    else if (crashAttr === 'road_surf_code')
        cols = [
    ...
    else if (crashAttr === 'road_horiz_align_code')
        cols = [
    ...
    else if (crashAttr === 'road_grade_code')
        cols = [
    ...
    else if (crashAttr === 'light_cond_code')
        cols = [
...
function createLocationClause(queryString) {
    var whereClauses = [];
    if (queryString.sri !== null && queryString.sri !== undefined && queryString.sri !== 'null')
        whereClauses.push(`calc_sri = '${queryString.sri}'`);
    else if (queryString.countyCode !== null && queryString.countyCode !== undefined) {
        whereClauses.push(`mun_cty_co = '${queryString.countyCode}'`);
        if (queryString.muniCode !== null && queryString.muniCode !== undefined)
            whereClauses.push(`mun_mu = '${queryString.muniCode}'`);
    } else return '';
...
function createFilterClause(queryString) {
    ...
    if (queryString.moduleType === 'sunglare') {
        if (
            queryString.travelDirectionCodes !== null &&
            queryString.travelDirectionCodes !== undefined &&
            queryString.travelDirectionCodes !== 'null'
        ) {
...
function createLimitSortClause(queryString) {
    var limitClause = '';
    if (queryString.countyCode !== null || (queryString.countyCode == null && queryString.sri == null))
