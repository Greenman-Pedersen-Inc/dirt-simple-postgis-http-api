...
function getQueryCategories(juriLevel) {
    ...
    return queryCategories.slice(0, idx + 1).reverse();     // the graph data starts from muni, county, mpo, state
}
...
function createWhereClause(queryCategory, queryArgs) {
    var whereClauses = [];
    if (queryArgs.startYear && queryArgs.endYear) whereClauses.push(`yr_num BETWEEN ${queryArgs.startYear} AND ${queryArgs.endYear}`);
    if (queryArgs.startTime !== '0000' && queryArgs.endTime !== '2400') whereClauses.push(`(hour >= '${queryArgs.startTime}' and hour <= '${queryArgs.endTime}')`);
    if (queryArgs.crashType !== '00') whereClauses.push(`crash_type = '${queryArgs.crashType}'`);

    if (queryCategory === 'mpo') {
        if (queryArgs.jurisdictionLevel.toLowerCase() === 'county' || queryArgs.jurisdictionLevel.toLowerCase() === 'municipality') {
