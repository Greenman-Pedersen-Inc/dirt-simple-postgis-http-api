// age_breakdown: Gets summation of pedestrian and cyclist person counts by age group
...
const sql = (queryArgs) => {
    var sql = `
    SELECT 
    SUM(CASE WHEN age < 16 THEN 1 ELSE 0 END) "<16",
    SUM(CASE WHEN age >= 16 AND age <= 25 THEN 1 ELSE 0 END) "16_25",
    SUM(CASE WHEN age >= 26 AND age <= 40 THEN 1 ELSE 0 END) "26_40",
    SUM(CASE WHEN age >= 41 AND age <= 64 THEN 1 ELSE 0 END) "41-64",
    SUM(CASE WHEN age >= 65 THEN 1 ELSE 0 END) ">65"
    FROM public.ard_pedestrians_partition
...
// *---------------*
const schema = {
    description: 'Gets summation of pedestrian and cyclist person counts by age group.',
    tags: ['ped-dashboard'],
    summary: 'Gets summation of pedestrian and cyclist person counts by age group.',
    querystring: {
        startYear: {
            type: 'string',
...
module.exports = function (fastify, opts, next) {
    fastify.route({
        method: 'GET',
        url: '/ped-dashboard/age-breakdown',
        schema: schema,
